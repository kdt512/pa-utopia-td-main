...
export class GameAudioAdapter extends Component {
  ...
  @property(AudioClip) public lazer: AudioClip;
  ...
  public init(levelManager: LevelManager): void {
    ...
    this.levelManager.Parking.onTapFailSound.on(() => {
      this.audioPlayer.playSound(this.failSound);
    }, this);
    this.levelManager.Parking.onCarStartSound.on(() => {
      this.audioPlayer.playSound(this.carSound);
    }, this);
    this.levelManager.Parking.onMatch.on(() => {
      this.audioPlayer.playSound(this.matchSound);
    }, this);
  }
}
